<template>
  ...
  <div class="mx-4 my-4">
    <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
      <div v-for="(item, index) in stats" :key="index" class="relative bg-white pt-5 px-4 pb-12 sm:pt-6 sm:px-6 shadow rounded-lg overflow-hidden">
        <dt>
        ...
        <dd class="ml-16 pb-6 flex items-baseline sm:pb-7">
          ...
          <div class="absolute bottom-0 inset-x-0 bg-gray-50 px-4 py-4 sm:px-6">
            <div class="text-sm">
              <Link :href="route(item.link)" class="font-medium text-orange-600 hover:text-orange-500">
                View all
                <span class="sr-only">
                  {{ item.name }} stats
                </span>
